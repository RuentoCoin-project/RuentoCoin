...
#if defined(NDEBUG)
# error "RuentoCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x890d6080ae238d14ffa846fa3bf48b2aec1c1e26f0f105591ab3a0db1be98407");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RuentoCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RuentoCoin Signed Message:\n";
...
{
    // RuentoCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 900000); // RuentoCoin: 100k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 24 * 60 * 60; // RuentoCoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // RuentoCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // RuentoCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // RuentoCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8dc43e48b4c4fd1ad4f35e0c31824bac5cca66393ac2c3ecb904809646a6348c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12/03/2021 From the core to the MOON! ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex(" 04ece4167a1f2e98816267e7925664c672ff4c26c09a2cb20d0534690942458d413dcf8396ea79756d114f7a5b5bbc30132533e3dc3b72f4fb77ff359d4479fd1a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615518183;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084835222;

        if (fTestNet)
        {
            block.nTime    = 1615518163;
            block.nNonce   = 386353650;
        } 
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x62d03c65d5cc6c024cd0f3d4fbbeaf8e4e805e40bb4ce36a019d43e3c8b9fe72"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xcc, 0xbe, 0xdf }; // RuentoCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RuentoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RuentoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RuentoCoinMiner(CWallet *pwallet)
{
    printf("RuentoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("RuentoCoin-miner");
    ...
    try { loop {
        ...
        printf("Running RuentoCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RuentoCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RuentoCoinMiner, pwallet));
}
